...
Each profile starts a real server, generates the snapshot, parses all JSON files, verifies the manifest and profile capabilities, validates global alias targets, and rejects required registries that unexpectedly produce empty output.